...
if __name__ == '__main__':
    ...
    snapshot_download(llava_model, local_dir=os.path.join(models_dir, model_folder), local_dir_use_symlinks=False)

    print(f'Downloading LLaVA CLIP model: {llava_clip_model}')
    model_folder = llava_clip_model.split('/')[1]
    snapshot_download(llava_clip_model, local_dir=os.path.join(models_dir, model_folder), local_dir_use_symlinks=False)

    print(f'Downloading SDXL CLIP model: {sdxl_clip_model}')
    model_folder = sdxl_clip_model.split('/')[1]
    snapshot_download(sdxl_clip_model, local_dir=os.path.join(models_dir, model_folder), local_dir_use_symlinks=False)
