...
class FaceRestoreHelper(object):
    ...
    def get_inverse_affine(self, save_inverse_affine_path=None):
        ...
        for idx, affine_matrix in enumerate(self.affine_matrices):
            print(f"DEBUG HELPER: Calculating inverse affine for face {idx}. Original Matrix:\\n{affine_matrix}")
            inverse_affine = cv2.invertAffineTransform(affine_matrix)
            print(f"DEBUG HELPER: Calculated inverse affine for face {idx} (NO extra scaling applied):\\n{inverse_affine}")
    ...
    def paste_faces_to_input_image(self, save_path=None, upsample_img=None, draw_box=False, face_upsampler=None):
        ...
        debug_dir = os.path.join('outputs', 'debug_paste')
        os.makedirs(debug_dir, exist_ok=True)
        dbg_timestamp = int(time.time() * 1000)

        if upsample_img is not None:
            h_up, w_up = upsample_img.shape[:2]
        else:
            h_up, w_up = int(h * self.upscale_factor), int(w * self.upscale_factor)
            upsample_img = cv2.resize(self.input_img, (w_up, h_up), interpolation=cv2.INTER_LINEAR)

        # Ensure upsample_img is float32 for blending
        upsample_img = upsample_img.astype(np.float32)

        dbg_base_name = f"{dbg_timestamp}_face{len(self.restored_faces)}"
        imwrite(upsample_img.astype(np.uint8), os.path.join(debug_dir, f'{dbg_base_name}_dbg_0_before_paste.png'))
        ...
        for face_idx, (restored_face, inverse_affine) in enumerate(zip(self.restored_faces, self.inverse_affine_matrices)):
            ...
            if restored_face.shape[2] == 4:
                restored_face = restored_face[:, :, :3]

            inv_restored = cv2.warpAffine(restored_face, inverse_affine, (w_up, h_up))
            imwrite(inv_restored.astype(np.uint8), os.path.join(debug_dir, f'{dbg_base_name}_face{face_idx}_dbg_1_warped.png'))
            ...
            inv_mask_erosion = cv2.erode(inv_mask, np.ones((erosion_kernel_size, erosion_kernel_size), np.uint8))

            # --- Ensure mask and face are 3-channel for blending
            if inv_mask_erosion.ndim == 2:
                inv_mask_erosion_3 = np.stack([inv_mask_erosion]*3, axis=-1)  # shape (H, W, 3)
            else:
                inv_mask_erosion_3 = inv_mask_erosion

            if inv_restored.shape[2] == 4:
                inv_restored = inv_restored[:, :, :3]
            if upsample_img.shape[2] == 4:
                upsample_img = upsample_img[:, :, :3]

            pasted_face = inv_mask_erosion_3 * inv_restored
            imwrite(pasted_face.astype(np.uint8), os.path.join(debug_dir, f'{dbg_base_name}_face{face_idx}_dbg_2_pasted_face.png'))
            ...
            if inv_soft_mask.ndim == 2:
                inv_soft_mask_3 = np.stack([inv_soft_mask]*3, axis=-1)
            else:
                inv_soft_mask_3 = inv_soft_mask

            imwrite((inv_soft_mask * 255).astype(np.uint8), os.path.join(debug_dir, f'{dbg_base_name}_face{face_idx}_dbg_3_soft_mask.png'))

            # --- Blend on top of background, preserving background everywhere else
            upsample_img = inv_soft_mask_3 * inv_restored + (1 - inv_soft_mask_3) * upsample_img

        upsample_img = np.clip(upsample_img, 0, 255)
        if upsample_img.dtype != np.uint8:
